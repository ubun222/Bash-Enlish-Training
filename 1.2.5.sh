...
(dirname $target > /dev/tty) >&/dev/null
key=$?
...
# for
read -p 输入y验证词表 verify
if [[ $verify = y || $verify = Y  ]];then

echo 验证词表中...
sha1=$(echo $txt | shasum)
sha2=$(echo $alldata | shasum)
sha3=$(echo $txt | sha1sum)
sha4=$(echo $alldata | sha1sum)
echo ${sha1:-shasum命令不存在}  #$txt
